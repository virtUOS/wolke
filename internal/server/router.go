...
func New(cfg *config.Config, deps Deps) (http.Handler, error) {
	...
	r.Use(securityHeaders(buildCSP(originOf(cfg.Branding.AssistantWidgetURL), originOf(cfg.OIDC.IssuerURL))))
...
func mountAuthenticated(r chi.Router, deps Deps, spaHandler http.Handler) {
	...
	// Origin header. The body limit applies as everywhere; the write rate
	// limit deliberately doesn't — every IdP logout arrives from one IP and a
	// 429 is never retried, so the route carries its own generous limit.
	backchannelLimiter := newKeyedLimiter(backchannelRatePerMinute)
	r.With(backchannelLimiter.middleware).Post(backchannelLogoutPath, deps.Auth.BackchannelLogout)
	...
	r.Group(func(pr chi.Router) {
		...
		if deps.Favorites != nil {
			if deps.Catalog != nil {
				...
				pr.With(requireUserJSON).Post("/api/favorites/items", addFavorite(deps.Favorites, deps.Catalog, vis, deps.Metrics))
			...
			pr.With(requireUserJSON).Delete("/api/favorites/items", removeFavorite(deps.Favorites, deps.Catalog, vis, deps.Metrics))
		}
